...
def create_resume(lst,item):
    ...
    resum.to_csv("resume_output/"+str(item)+".csv", sep=';')
...
class App:
    def __init__(self,window0, window_title, video_source="video_input/VIDEO1.mp4"):
        self.filename=None
        ...
        self.window0.title(window_title)

        self.btn_select=tkinter.Button(self.window0, text="Select folder", width=15, command=self.open_dir)
        self.btn_select.pack(anchor=tkinter.NW, expand=True)
        self.btn_nw=tkinter.Button(self.window0, text="Next Video", width=15, command=self.new_window)
        self.btn_nw.pack(anchor=tkinter.NW, expand=True)
        ...
        # self.update()
        #self.window0.mainloop()
        #self.button.wait_variable(self.open_file())
        
 
    def new_window(self):
        self.v_num+=1
        print(self.v_num)
        if (self.v_num>=len(self.onlyfiles)):
            self.window = tkinter.Toplevel(self.window0)
            self.window.title("Error") 
            self.window.geometry("200x200")
  
            # A Label widget to show in toplevel
            tkinter.Label(self.window, text="No more videos to process !").pack()
        else:
            ...
            # A Label widget to show in toplevel 
            tkinter.Label(self.window,text ="This is a new window").pack() 
            ...
            cv2.createTrackbar( 'end'  , "Parameters", 10, length, self.vid.onChange)
            ...
            cv2.createTrackbar('OFF/ON', 'Parameters', 1, 1, self.vid.check_box)
            ...
            self.switch= cv2.getTrackbarPos('OFF/ON','Parameters')
            ...
            #output_movie = cv2.VideoWriter(video_name.split('_')[0]+'_output/'+video_name.split('/')[1].split('.')[0]+'_output.avi', fourcc, fps, (width, height) )
            self.output_movie = cv2.VideoWriter('video_output/'+ self.onlyfiles[self.v_num].split('.')[0]+'_output.avi', self.fourcc, self.fps, (self.width, self.height) )
    ...
    def open_dir(self):
        ...
        self.onlyfiles = [f for f in listdir(self.directory)]
    ...
    def open_file(self):
        self.filename= str(self.directory+"/"+self.onlyfiles[self.v_num])
        print(self.filename)
    ...
    def update(self):
        ...
        if(frame is None):
            ...
            res=create_resume(self.lst,self.onlyfiles[self.v_num].split(".")[0])
            ...
            if not faceBoxes:
                ...
                for faceBox in faces:
                   ...
                   # print(face)
                    ...
                    if (gender=='Homme'):
                        cv2.rectangle(resultImg,(faceBox[1],faceBox[0]),(faceBox[3],faceBox[2]),(0,0,255),2)
                    else:
                        cv2.rectangle(resultImg,(faceBox[1],faceBox[0]),(faceBox[3],faceBox[2]),(255,0,0),2)
                    text = f'{name}\n{gender}\n{apparent_predictions}\n{emotion_dict[maxindex]}'
                    y0, dy = int(faceBox[0]-25*3.5), 25
                    for i, line in enumerate(text.split('\n')):
                        y = y0 + i*dy
                        cv2.putText(resultImg, line, (faceBox[3], y ), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2, cv2.LINE_AA)
                    ...
                    if (cpt==len(faces)):
                        if ret:
                            ...
                            self.output_movie.write(cv2.cvtColor(resultImg, cv2.COLOR_BGR2RGB))
                            resultImg=cv2.resize(resultImg,(913,515))
                            cv2.putText(resultImg, str(self.k)+'/'+str(self.end-self.start), (0, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 2)
